...
**Endpoints:**
```
POST   /agent/generate-artifact  { prompt, slug?, roles? }  → one-shot create (thin wrapper around /chat-artifact)
POST   /agent/chat-artifact  { messages, slug?, roles? }    → multi-turn create/update, returns updated message history
...
2. There is no provider/model picker — every turn uses the same fixed `--model anthropic/claude-sonnet-5` (the `OPENCODE_MODEL` constant in `src/config.ts`; change the constant, not a request field or env var, to move models). Provider *credentials* are still opencode's own concern (`opencode auth login`, or `ANTHROPIC_API_KEY` inherited from this process's environment), not something this service holds.
...
**Config** (`src/config.ts`): `PORT` (default `5102` — chosen to avoid a `5002` collision with another service on a shared host), `ARTIFACTS_SERVER_URL`, `ARTIFACTS_ROOT` (default `../artifacts-server/artifacts`), `OPENCODE_BIN` (default `opencode`, resolved via `PATH`), `OPENCODE_TIMEOUT_SECONDS` (default `900`), plus `TOOL_SERVICE_URL` — read by the `get_tools` opencode tool (inherited into the opencode subprocess's environment), defaulting to `http://localhost:5104` if unset. The model itself (`OPENCODE_MODEL`, `anthropic/claude-sonnet-5`) is a fixed constant, not an env var — there is no provider/model switching. This service holds no database credential of its own, generation-time or otherwise — it never did anything but proxy opencode's own subprocess to the filesystem, and now opencode's only outbound network dependency for data-shape knowledge is `tool-service`'s open, metadata-only `GET /tools`.
...
A Next.js (App Router) app. There's no role picker anywhere — a user logs in with real `tool-service` credentials, and their role is whatever the JWT's `/auth/verify` resolution reports. The artifact list, the sidebar, and the chat page all just reflect that.
...
1. The artifact's JS posts `{ source: 'artifact-data-bridge', type: 'request', requestId, tool, args }` to `window.parent` — it holds no credentials and cannot reach any backend on its own (sandboxed, no `allow-same-origin`, and `connect-src 'none'` blocks any outbound call it might attempt anyway). Any `confirmed` field an artifact's message includes is read nowhere downstream — see the next point.
2. `useArtifactDataBridge.ts` validates the sender via `event.source === iframe.contentWindow` (**not** `event.origin`, since a sandboxed iframe's origin is the opaque string `"null"`). With no session, it short-circuits to a `401`/`UNAUTHENTICATED` response without calling anything else. Otherwise it looks up the requested tool's real `mutates`/`destructive` metadata via `useToolCatalog.ts` (which fetches this app's own `GET /api/tools` → `tool-service`'s `GET /tools`, cached module-wide for the session) — an unresolved lookup, whether the catalog hasn't loaded yet or the name isn't a real tool, fails **closed** and is treated as requiring confirmation, never as safe to skip. If confirmation is required, the hook blocks on a `pendingConfirmation` state that drives `ConfirmMutationDialog.tsx` (mounted alongside the iframe in `ArtifactFrame.tsx`) and does not proceed until the user actually clicks Confirm there; Cancel (or dismissing the dialog) resolves with a `{ ok: false, code: 'USER_CANCELLED' }` response and the request never reaches the BFF at all. Only once that's settled does it make the real request: `fetch('/api/tools/<tool>', { method: 'POST', headers: { Authorization: Bearer <session token> }, body: { args, confirmed } })`, using the logged-in user's own session — `confirmed` here is always this platform-level outcome, never whatever value the artifact's own message claimed.
3. `app/api/tools/[name]/route.ts` is a thin BFF proxy to `tool-service`'s `POST /tools/:name/execute` (`lib/api/tool-service-client.ts`'s `executeTool`) — no CORS — deliberately, as defense-in-depth against any artifact that somehow tried to call them directly. (`login`/`register` get their own dedicated routes, §above, since they're unauthenticated and conceptually distinct from an authenticated data-bridge call.) `app/api/tools/route.ts` (`GET`, no auth) is the matching catalog proxy backing `useToolCatalog.ts` — public metadata, same as `tool-service` itself treats it.
...
- `lib/api/tool-service-client.ts`, `artifact-agent-service-client.ts`, `db-agent-service-client.ts`, `artifacts-catalog-client.ts` — server-only (`import 'server-only'`) clients for each backend; `lib/api/catalog-client.ts`, `unified-chat-client.ts`, `chat-sessions-client.ts`, `db-chat-client.ts`, `skills-client.ts`, `session-client.ts` — matching browser-side clients that call this app's own BFF routes
- `app/api/tools/{login,register}/route.ts` + `app/api/tools/[name]/route.ts` + `app/api/tools/route.ts` (GET, the tool catalog), `app/api/artifacts/route.ts` (list) + `app/api/artifacts/[...slug]/route.ts` (rename/delete), `app/api/artifact-proxy/[...path]/route.ts`, `app/api/chat/route.ts` (the unified, session-aware chat endpoint — see below), `app/api/chat/sessions/route.ts` + `app/api/chat/sessions/[id]/route.ts`, `app/api/skills/{route.ts,[name]/route.ts}` — the BFF routes bridging browser ↔ each backend
- `lib/session/session-context.tsx` — React context holding the `tool-service` **auth** session (`useSession()`), plus `login`/`register`/`logout`. Deliberately a different concept from a **chat session** (`ChatSession`, this app's own persisted conversation thread — see below); the two are never conflated in naming or code.
- `hooks/useArtifactDataBridge.ts` — the postMessage mediator described above, including the confirmation gate
- `hooks/useToolCatalog.ts` — fetches `GET /api/tools` once (module-scope cache across mounts/rerenders) and exposes `getToolMeta(name)`, the real authority `useArtifactDataBridge.ts` uses to decide whether a call needs confirmation
...
- `components/ArtifactSelector.tsx` (a sidebar list of pages, not a dropdown), `ArtifactFrame.tsx` (accepts a `reloadNonce` prop to force a remount/reload independent of `src`; also mounts `ConfirmMutationDialog.tsx` alongside the iframe), `components/ConfirmMutationDialog.tsx` (the platform's own mutation-confirmation UI, driven by `useArtifactDataBridge.ts`'s `pendingConfirmation` state — distinct styling when the tool is `destructive`), `ArtifactViewer.tsx` (its sidebar links to `/chat`), `components/auth/AuthWidget.tsx` + `ProfileMenu.tsx` (email/role/logout, opens as a popup from a profile chip — no `RoleSwitcher`, roles are never user-selectable) — composed in `app/page.tsx`
...
### Unified Chat page (`/chat`)

`/chat` and the old `/db-chat` page are one page: a single message thread that transparently routes each turn to either the artifact agent or the database agent, and persists every turn server-side as a `ChatSession`/`ChatMessage` — this app's **first-ever database credential**, fully separate from `tool-service`'s ERP Postgres (see below). `/db-chat` now 301/307-redirects to `/chat` (`next.config.js`'s `redirects()`) rather than 404ing.

- `app/chat/page.tsx` → `components/chat/ChatPage.tsx` — owns the conversation state (`messages`, the active `sessionId`, the target artifact `slug`, selected skills, the live preview pane, the session sidebar's open/closed state)
- `components/chat/ChatMessageList.tsx`, `ChatComposer.tsx`, `ExistingArtifactsPanel.tsx`, `SessionSidebar.tsx` (the sidebar list of the caller's own past sessions — same visual convention as `ArtifactSelector.tsx`) — presentational pieces
- `components/chat/SkillSelector.tsx` — a row of toggleable chips, one per skill; selections persist across turns until toggled off. Still meaningful only for turns that route to the artifact agent, but no longer gated behind a separate page.
- `components/chat/SkillsPanel.tsx` — inline create/edit/delete UI for skills, calling `/api/skills`
- `components/db-chat/DynamicTable.tsx`/`DynamicChart.tsx`/`DynamicCard.tsx`/`DynamicForm.tsx` — render a database-routed turn's structured result (table/chart/card) or a pending write's confirmation form
- `lib/chat/types.ts`, `lib/db-chat/types.ts`, `lib/skills/types.ts`, `lib/unified-chat/types.ts` (`AgentType = 'artifact' | 'db'`), `lib/chat-sessions/types.ts` — the wire types shared between the proxies and the UI

**Routing (unchanged from before this pass):** `lib/unified-chat/agent-router.ts`'s `routeToAgent()` classifies the new user message alone (no history) via a cheap Haiku call, defaulting to `'db'` on any API error — this decision logic itself is out of scope for the session-persistence work below.

**Session persistence (`POST /api/chat`):** the client no longer resends the full transcript on every turn. It sends `{ sessionId?, message }`; the server reconstructs prior history from `ChatMessage` rows.
1. Resolve `sessionId` (create a `ChatSession` if omitted, titled from the first ~50 characters of `message`) and confirm `session.userId` matches the caller's own identity — see auth below. A user can never read or append to another user's session (`403`); an unknown `sessionId` is `404`.
2. Load prior `ChatMessage` rows for the session and reconstruct the `{role, content}` history both backend agents expect.
3. Persist the new user message immediately, **before** calling either backend agent, so a request that fails partway through still leaves it recorded.
4. Run `routeToAgent()` + the matching backend call exactly as before. For an artifact-routed turn, the `slug` sent to `artifact-agent-service` is derived from the session's own history (the most recent message with `route: 'artifact'` and a non-null `artifactSlug`), not client-held state.
5. Persist the assistant's reply (`role: 'assistant'`, `route`, `artifactSlug` if applicable), and return `{ sessionId, reply, route, artifactSlug?, artifact?, db? }` — `artifact`/`db` carry the same rich payload the pre-persistence endpoint returned (`url_path`/`preview_url` for the preview pane; the full `DbChatResponsePayload` for `DynamicTable`/`DynamicChart`/`DynamicCard`/`DynamicForm`), so rendering is unchanged even though transport is.

Because a `ChatSession` belongs to a user, every turn — artifact- or database-routed — now requires the caller's `tool-service` access token (previously only database-routed turns did); this app never decodes/verifies that token itself (see the security table below) — `lib/chat-sessions/auth.ts`'s `resolveUserId()` calls `tool-service`'s existing authenticated `whoami` tool instead, the same way every other token-bearing request here is authenticated.

**Session management:** `GET /api/chat/sessions` (the caller's own sessions, newest first), `GET /api/chat/sessions/[id]` (full history + the session's current artifact slug, for resuming the preview pane), `PATCH .../[id]` (rename), `DELETE .../[id]` (cascades to its `ChatMessage` rows via the FK's `onDelete: Cascade`) — all behind the same ownership check as `POST /api/chat`. `lib/chat-sessions/store.ts` holds the Prisma-backed reads/writes; `lib/api/chat-sessions-client.ts` is the matching browser client the sidebar uses.

**This app's database:** `apps/artifacts-viewer/prisma/schema.prisma`, its own migrations folder, and a custom Prisma Client `output` path (`src/generated/prisma-client`, imported via a relative path rather than `@prisma/client`) — this npm-workspaces monorepo hoists `@prisma/client` to the root `node_modules`, and a default-output client here would silently overwrite (or be overwritten by) `tool-service`'s own generated client at that same shared location. `SESSION_DATABASE_URL` (never `DATABASE_URL`) points it at a **separate Postgres database** — `docker-compose.yml`'s `session-postgres` service by default, or an external instance — that only `artifacts-viewer` ever holds a credential for; it is never joined or queried alongside `tool-service`'s ERP database in application code. Chat session history only — `ChatSession`/`ChatMessage` — nothing else lives here.
...
| An artifact could write data without a real confirmation — e.g. badly-generated JS that sets `confirmed: true` itself with no dialog of its own | Not the artifact's call to make: `useArtifactDataBridge.ts` looks up the tool's real `mutates`/`destructive` metadata via `useToolCatalog.ts` (`tool-service`'s own `GET /tools`) and blocks on its own `ConfirmMutationDialog.tsx` before a mutating request is ever sent; the `confirmed` field on the artifact's message is read nowhere in this path. An unresolved catalog lookup (still loading, or an unknown tool name) fails **closed** — treated as requiring confirmation, never as safe to skip |
...
| 2. Tool layer | Each `ToolDefinition.handler` in-process; the HTTP contract (`tools.router.ts`'s error-code translation, the `confirmed` gate, auth) | `services/tool-service/test/tools/` (in-process) and `services/tool-service/test/http/` (over HTTP) — **built, Batch A–F coverage closed** (F: `start_stock_count`/`submit_count_line`/`submit_stock_count`/`approve_stock_count`/`reject_stock_count` — the count → submit → owner-approve/reject lifecycle, including the `trg_guard_count_adjustment` status-flip-before-movements ordering proof in both Layer 1 and Layer 2) (`npm run test:layer2:tools`/`test:layer2:http`/`test:layer2`, see that project's README) | Jest + Prisma for the first (`test/infra/getTestPrismaClient`); `supertest` against `src/app.ts`'s real `createApp()`, booted on an ephemeral port, for the second; a fresh Testcontainers Postgres per test file, not a shared/reset database |
