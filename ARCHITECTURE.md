...
- `app/chat/page.tsx` → `components/chat/ChatPage.tsx` — owns the conversation state (`messages`, the target `slug`, selected skills, the live preview pane); there's no provider/model picker, opencode always runs with a fixed model (see §4)
- `components/chat/ChatMessageList.tsx`, `ChatComposer.tsx`, `ExistingArtifactsPanel.tsx` — presentational pieces for the conversation and artifact-switching
