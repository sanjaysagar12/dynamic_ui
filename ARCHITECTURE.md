...
**This app's database:** `apps/artifacts-viewer/prisma/schema.prisma`, its own migrations folder, and a custom Prisma Client `output` path (`src/generated/prisma-client`, imported via a relative path rather than `@prisma/client`) — this npm-workspaces monorepo hoists `@prisma/client` to the root `node_modules`, and a default-output client here would silently overwrite (or be overwritten by) `tool-service`'s own generated client at that same shared location. `VIEWER_DATABASE_URL` (never `DATABASE_URL`) points it at a **separate Postgres database** — `docker-compose.yml`'s `viewer-postgres` service by default, or an external instance — that only `artifacts-viewer` ever holds a credential for; it is never joined or queried alongside `tool-service`'s ERP database in application code. Chat session history only — `ChatSession`/`ChatMessage` — nothing else lives here.
